...
const mime = {
  ...
  '.md':'text/markdown; charset=utf-8', '.csv':'text/csv; charset=utf-8', '.xml':'application/xml; charset=utf-8', '.log':'text/plain; charset=utf-8',
  '.webp':'image/webp', '.bmp':'image/bmp', '.mp4':'video/mp4', '.mp3':'audio/mpeg', '.wav':'audio/wav'
};
...
function defaultDb(){
  return {
    ...
    system_config:{
      ...
      wallpaper:'linear-gradient(135deg,#0f172a,#1e1b4b 45%,#0b1120)', accent:'#7c3aed', theme:'glass', ui_scale:'0.88'
    },
    desktop_icons:[
...
function normalizeRelDir(value){
  ...
  rel = rel.split('/').filter(Boolean).filter(x=>x!=='.' && x!=='..').join('/');
  return rel;
}
function resolveUploadPath(rel){
  const clean = normalizeRelDir(rel);
  const abs = path.resolve(UPLOADS_DIR, clean);
  const base = path.resolve(UPLOADS_DIR);
  if(abs !== base && !abs.startsWith(base + path.sep)) throw new Error('Ruta no permitida');
...
function folderSizeSafe(abs){
  try{
    const st = fs.statSync(abs);
    if(st.isFile()) return st.size;
    let total = 0;
    for(const n of fs.readdirSync(abs)){
      const p = path.join(abs,n); const s = fs.statSync(p);
...
function listFolder(rel){
  const {abs, rel:cleanRel} = resolveUploadPath(rel);
  fs.mkdirSync(abs, {recursive:true});
  const entries = fs.readdirSync(abs, {withFileTypes:true}).map(d=>{
    const full = path.join(abs, d.name); const st = fs.statSync(full); const isDir=d.isDirectory();
    const itemRel = (cleanRel ? '/' + cleanRel + '/' + d.name : '/' + d.name).replace(/\/+/g,'/');
    return { name:d.name, path:itemRel, kind:isDir?'folder':'file', ext:isDir?'':path.extname(d.name).toLowerCase(), size:isDir?folderSizeSafe(full):st.size, modified_at:st.mtime.toISOString(), created_at:st.birthtime.toISOString(), mime:isDir?'inode/directory':(mime[path.extname(d.name).toLowerCase()] || 'application/octet-stream') };
  }).sort((a,b)=>(a.kind===b.kind?0:(a.kind==='folder'?-1:1)) || a.name.localeCompare(b.name,'es'));
  const parent = cleanRel ? '/' + cleanRel.split('/').slice(0,-1).join('/') : null;
  return { current: cleanRel ? '/' + cleanRel : '/', parent, entries };
}
function isTextLike(file){
  const ext = path.extname(file).toLowerCase();
  return ['.txt','.md','.csv','.json','.js','.css','.html','.xml','.log','.yaml','.yml','.sql'].includes(ext);
}
...
async function handleApi(req, res, pathname){
  ...
  try{
    ...
    if(req.method==='POST' && pathname==='/api/change-password'){
      ...
      if(!u || !verifyPassword(body.oldPassword || body.currentPassword, u.password_hash)) return bad(res,'Contraseña actual incorrecta',403);
    ...
    if(req.method==='GET' && pathname==='/api/fs/list'){
      ...
      return sendJson(res,200,{ok:true,...listFolder(url.searchParams.get('dir') || '/')});
    }
    if(req.method==='GET' && pathname==='/api/fs/view'){
      const url = new URL(req.url, `http://${req.headers.host}`);
      const {abs, rel} = resolveUploadPath(url.searchParams.get('path') || '/');
      if(!fs.existsSync(abs) || !fs.statSync(abs).isFile()) return notFound(res);
      const ext = path.extname(abs).toLowerCase();
      const type = mime[ext] || 'application/octet-stream';
      res.writeHead(200, {'Content-Type': type, 'Content-Disposition':`inline; filename="${encodeURIComponent(path.basename(abs))}"`});
      fs.createReadStream(abs).pipe(res); return;
    }
    if(req.method==='GET' && pathname==='/api/fs/text'){
      const url = new URL(req.url, `http://${req.headers.host}`);
      const {abs} = resolveUploadPath(url.searchParams.get('path') || '/');
      if(!fs.existsSync(abs) || !fs.statSync(abs).isFile()) return notFound(res);
      if(fs.statSync(abs).size > 2*1024*1024) return bad(res,'Archivo de texto demasiado grande para vista previa');
      return send(res,200,fs.readFileSync(abs),{'Content-Type':'text/plain; charset=utf-8'});
    }
    if(req.method==='POST' && pathname==='/api/fs/mkdir'){
      const body = await readJson(req); const base = resolveUploadPath(body.dir || '/'); const name = cleanFileName(body.name || 'Nueva carpeta');
      const target = resolveUploadPath((base.rel ? '/' + base.rel : '') + '/' + name); fs.mkdirSync(target.abs,{recursive:true});
      return sendJson(res,200,{ok:true,path:target.rel ? '/' + target.rel : '/'});
    }
    if(req.method==='DELETE' && pathname==='/api/fs/delete'){
      const body = await readJson(req); const target = resolveUploadPath(body.path || '/');
      if(target.rel === '') return bad(res,'No se puede borrar la raíz');
      if(!fs.existsSync(target.abs)) return notFound(res);
      fs.rmSync(target.abs,{recursive:true,force:true});
      db.files=(db.files||[]).filter(x=>!String(x.path||'').startsWith(target.abs)); saveDb(db);
    ...
    if(req.method==='POST' && pathname==='/api/files/upload'){
      ...
      const url = new URL(req.url, `http://${req.headers.host}`);
      const dirInfo = resolveUploadPath(url.searchParams.get('dir') || '/'); fs.mkdirSync(dirInfo.abs,{recursive:true});
      const original=cleanFileName(file.filename); const stored=original; let dest=path.join(dirInfo.abs, stored);
      if(fs.existsSync(dest)){ const ext=path.extname(original); const base=path.basename(original,ext); dest=path.join(dirInfo.abs, `${base}_${Date.now()}${ext}`); }
      fs.writeFileSync(dest, file.data);
      const row={id:nextId(db,'files'), name:path.basename(dest), original_name:path.basename(dest), stored_name:path.basename(dest), rel_path:relFromAbs(dest), path:dest, type:file.contentType||mime[path.extname(dest).toLowerCase()]||'application/octet-stream', size:file.data.length, size_bytes:file.data.length, created_at:now(), updated_at:now()};
