...
function getBasePathFromWebdavUrl(account) {
  try {
    return normalizeDavPath(new URL(account.base_url || "http://local/").pathname || "/").replace(/\/+$/, "") || "/";
  } catch {
    return "/";
  }
}

function pathFromWebdavHref(account, hrefRaw, currentPath, name) {
  const current = normalizeDavPath(currentPath || account.root || "/");
  const hrefPath = hrefToPathname(hrefRaw).replace(/\/+$/, "") || "/";
  const basePath = getBasePathFromWebdavUrl(account).replace(/\/+$/, "") || "/";

  let candidate = "";

  if (basePath !== "/" && (hrefPath === basePath || hrefPath.startsWith(basePath + "/"))) {
    candidate = hrefPath.slice(basePath.length) || "/";
  } else {
    const root = normalizeDavPath(account.root || "/").replace(/\/+$/, "") || "/";
    if (root !== "/") {
      const idx = hrefPath.indexOf(root + "/");
      if (idx >= 0) candidate = hrefPath.slice(idx);
      else if (hrefPath === root) candidate = root;
    }
  }

  candidate = normalizeDavPath(candidate || "");

  if (candidate !== "/") return candidate;
  return joinDavPath(current, name);
}

function parseWebdavList(xml, currentPath, account = {}) {
  const responses = String(xml || "").match(/<[^>]*:?response[\s\S]*?<\/[^>]*:?response>/gi) || [];
  const current = normalizeDavPath(currentPath || account.root || "/");
  const currentNoSlash = current.replace(/\/+$/, "") || "/";
  const out = [];
  ...
  for (const block of responses) {
    ...
    const decodedHrefPath = hrefToPathname(hrefRaw);
    const hrefNoSlash = decodedHrefPath.replace(/\/+$/, "");
    let name = hrefNoSlash.split("/").filter(Boolean).pop() || "";
    ...
    const pathValue = pathFromWebdavHref(account, hrefRaw, current, name);
    const pathNoSlash = normalizeDavPath(pathValue).replace(/\/+$/, "") || "/";

    // El primer <response> de PROPFIND Depth:1 suele ser la carpeta actual; no debe mostrarse ni abrirse como elemento.
    if (pathNoSlash === currentNoSlash) continue;

    // Evita que algunos servidores devuelvan ancestros o rutas fuera del directorio solicitado.
    if (currentNoSlash !== "/" && !pathNoSlash.startsWith(currentNoSlash + "/")) {
      const fallback = joinDavPath(current, name);
      if (normalizeDavPath(fallback).replace(/\/+$/, "") === currentNoSlash) continue;
      out.push({ id: fallback, name, type: isDir ? "folder" : "file", path: fallback, size: isDir ? 0 : size, modified });
      continue;
    }
  ...
  for (const item of out) {
    const key = normalizeDavPath(item.path) + "|" + item.type;
    if (!seen.has(key)) {
      seen.add(key);
      unique.push(item);
    }
...
async function webdavList(account, remotePath) {
  ...
  return parseWebdavList(xml, pathValue, account);
}
